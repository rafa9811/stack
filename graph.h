...
/* Se añade una conexión (entre el nodo con id nId1 y el nodo con id nId2) al grafo, actualizando los atributos internos del grafo que sean necesarios. Se devuelve el grafo actualizado si todo ha ido bien, o NULL en otro caso */
...
/* Devuelve TRUE o FALSE según si los nodos correspondientes a los ids pasados como argumento están conectados en el grafo o no (existe una conexión entre nId1 y nId2, en ese sentido). Si hay algún error se devuelve FALSE */
...
/* Imprime en pf los datos de un grafo, devuelve el número de caracteres que se han escrito con éxito (mirar documentación de fprintf). El formato a seguir es: imprimir una primera línea con el número de nodos y conexiones, después, se imprime una línea por nodo, incluyendo la información del nodo y las conexiones con el resto de nodos como 1 o 0.
